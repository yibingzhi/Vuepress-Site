...
3. 线程不安全：多个线程同时操作可能导致数据不一致（可以用 Collections.synchronizedList 或 CopyOnWriteArrayList 替代）。
...
1. elementData：Object[] 类型的数组，真正存储 ArrayList 中的元素。由于是 Object 类型，可以存储任意对象（泛型 <E> 只是编译期约束）。
