...
Java虚拟机（JVM）是Java平台的核心组件，它是一个虚拟的计算机，负责执行Java字节码。JVM使得Java程序能够实现"一次编写，到处运行"的特性。
...
2. **ParNew收集器**：
   - Serial收集器的多线程版本
   - 除了使用多线程进行垃圾收集之外，其余行为包括Serial收集器可用的所有控制参数、收集算法、Stop The World、对象分配规则、回收策略等都与Serial收集器完全一样
